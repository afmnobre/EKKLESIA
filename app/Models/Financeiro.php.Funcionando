...
class Financeiro {
	...
	public function processarBaixa($data) {
		try {
			...
			// --- CORREÇÃO DO HORÁRIO ---
			// Se a data vier apenas como '2026-03-23', adicionamos a hora atual '15:30:22'
			$data_pagamento_completa = $data['data_pagamento'];
			if (strlen($data_pagamento_completa) <= 10) {
				$data_pagamento_completa .= ' ' . date('H:i:s');
			}

			// 0. BUSCAR O TIPO DA CONTA
			$stmtTipo = $this->db->prepare("SELECT financeiro_conta_tipo, financeiro_conta_descricao FROM financeiro_contas WHERE financeiro_conta_id = ?");
			...
			// 1. Atualiza o status da conta agendada (Usando a data completa)
			...
			// 2. Registra o Pagamento Detalhado (Usando a data completa)
			...
			// 4. Cria o Registro no Extrato (Usando a data completa)
			...
			$this->db->prepare($sql4)->execute([
				...
				$tipo,
				$data['valor'],
				$data_pagamento_completa, // Aqui garante que no extrato apareça a hora
				$desc
			]);
	...
	public function getMovimentacoesPorPeriodo($igrejaId, $dataInicio, $dataFim) {
		...
		$stmt = $this->db->prepare($sql);
		// Adicionamos o horário para cobrir o dia inteiro (00:00:00 até 23:59:59)
